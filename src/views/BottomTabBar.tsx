    ...
    render() {
        ...
        return (
            <TouchableWithoutFeedback
                ...
                testID={testID}
                hitSlop={{left: 0, right: 0, top: 0, bottom: 5}}
...
class TabBarBottom extends React.Component<BottomTabBarProps, State> {
    static defaultProps = {
        ...
        adaptive: isIOS11,
        safeAreaInset: {bottom: 'always', top: 'never'} as React.ComponentProps<typeof SafeAreaView>['forceInset'],
    };
            ...
            >
                <SafeAreaView style={tabBarStyle} forceInset={safeAreaInset}>
                    {routes
                        .filter((route, index) => {
                            if (hiddenRouteName) {
                                if (hiddenRouteName.split('|').indexOf(route.routeName) === -1) {
                                    return true
                                } else {
                                    hiddenRouteIndexs.push(index)
                                    return false
                                }
                            } else {
                                return true;
                        ...
                        .map((route, index) => {
                            let offset = 0;
                            for (let i = 0; i < hiddenRouteIndexs.length; i++) {
                                if (navigation.state.index > hiddenRouteIndexs[i]) {
                                    offset++
                                }
                            }
                            const focused = index === (navigation.state.index - offset);
